...
//        }
        ...
        for (int i = 0; i < size; i++) {
            for (int j = 0; j < size - 2; j++) {
                if (playingField[j][i] != EMPTY_FIELD &&
                        playingField[j][i] == playingField[j + 1][i] &&
                        playingField[j][i] == playingField[j + 2][i]) {
                    return true;
                }
            }
        }
        ...
        for (int i = 0; i < size - 2; i++) {
            for (int j = 0; j < size - 2; j++) {
                // Проверяем главную диагональ (лево-верх -> право-низ)
                if (playingField[i][j] != EMPTY_FIELD &&
                        playingField[i][j] == playingField[i+1][j+1] &&
                        playingField[i][j] == playingField[i+2][j+2]){
                    return true;
                }
                // Проверяем побочную диагональ (право-верх -> лево-низ)
                if (playingField[i][size - j - 1] != EMPTY_FIELD &&
                        playingField[i][size - j - 1] == playingField[i+1][size - j - 2] &&
                        playingField[i][size - j - 1] == playingField[i+2][size - j - 3]){
                    return true;
                }
            }
        }
